...
起動時に **2つの使い方（モード）** を選べます（選択は記憶され、後から「モード変更」で切替可能）。

- **この端末だけで使う**（ログイン不要）… データはこの端末のブラウザ（localStorage）に保存。設定ゼロですぐ使えます。他の端末とは共有されません。
- **ログインしてクラウド同期** … メールでログインすると、スマホ・PCなど複数端末で同じ記録を使えます。無料の [Supabase](https://supabase.com/) を使います（下記セットアップが必要）。

フロントは静的サイト（HTML / CSS / JavaScript）です。

> **どちらを選べばいい？**
> まず試すだけ・1台で使うなら「この端末だけ」。複数端末で同じ記録を見たいなら「クラウド同期」。
> クラウド同期は Supabase の設定（下記）が必要です。「この端末だけ」は設定不要です。
...
## クラウド同期のセットアップ（初回のみ・約5〜10分）

> 「この端末だけで使う」場合はこの章は不要です。`index.html` を開いて「この端末だけで使う」を選ぶだけで使えます。
...
- **この端末だけモード**: データはこの端末のブラウザ（localStorage）に保存されます。他の端末とは共有されず、ブラウザのデータを消去すると記録も消えます。
- **クラウド同期モード**: データは Supabase に保存され、ログインすればどの端末からでも同じ記録を見られます（ネット接続が必要）。
- 2つのモードのデータはそれぞれ別に保存されます（モードを切り替えても自動では統合されません）。
- どちらのモードでも、手元に残したいときは **CSVダウンロード** で書き出せます。
